import React, { useEffect, useState } from 'react';
import { fetchMarketSentiment, SentimentAnalysis } from '../services/geminiService';

const SentimentFeed: React.FC = () => {
  const [data, setData] = useState<SentimentAnalysis | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const loadSentiment = async () => {
    setLoading(true);
    setError(null);
    try {
      const sentimentData = await fetchMarketSentiment();
      setData(sentimentData);
    } catch (err) {
      setError("API Quota exceeded. Retrying later...");
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    // stagger load to prevent 429
    const timer = setTimeout(loadSentiment, 2500);
    const interval = setInterval(loadSentiment, 3600000);
    return () => {
      clearTimeout(timer);
      clearInterval(interval);
    };
  }, []);

  if (loading) {
    return (
      <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 animate-pulse h-full">
        <div className="h-6 bg-slate-700 rounded w-1/2 mb-4"></div>
        <div className="space-y-3">
          <div className="h-4 bg-slate-700 rounded w-full"></div>
          <div className="h-4 bg-slate-700 rounded w-5/6"></div>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="bg-slate-800 p-6 rounded-lg border border-red-900/30 h-full flex flex-col justify-center items-center text-center">
        <p className="text-red-400 text-sm mb-4">{error}</p>
        <button onClick={loadSentiment} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded text-xs transition-colors">
          Try Again
        </button>
      </div>
    );
  }

  return (
    <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 h-full flex flex-col">
      <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-700">
        <h3 className="text-white font-bold flex items-center space-x-2">
          <svg className="w-5 h-5 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10l4 4v10a2 2 0 01-2 2z" />
          </svg>
          <span>Market Sentiment Hub</span>
        </h3>
        <button onClick={loadSentiment} className="text-slate-500 hover:text-indigo-400 transition-colors">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
        </button>
      </div>

      <div className="flex-1 space-y-4">
        <div className={`p-3 rounded-md border text-center font-bold text-sm tracking-wide ${getSentimentStyles(data?.sentiment)}`}>
          CURRENT SENTIMENT: {data?.sentiment?.toUpperCase() || 'NEUTRAL'}
        </div>

        <div className="text-sm text-slate-300 leading-relaxed italic">
          "{data?.summary}"
        </div>

        {data?.sources && data.sources.length > 0 && (
          <div className="pt-4 border-t border-slate-700/50">
            <p className="text-[10px] uppercase font-bold text-slate-500 mb-2 tracking-widest text-left">Grounded Sources</p>
            <div className="space-y-1.5">
              {data.sources.map((src, idx) => (
                <a key={idx} href={src.uri} target="_blank" rel="noopener noreferrer" className="block text-xs text-indigo-400 hover:text-indigo-300 truncate text-left underline decoration-indigo-500/30">
                  {idx + 1}. {src.title}
                </a>
              ))}
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

function getSentimentStyles(s?: string) {
  if (s === 'Bullish') return 'bg-green-500/10 text-green-400 border-green-500/30';
  if (s === 'Bearish') return 'bg-red-500/10 text-red-400 border-red-500/30';
  return 'bg-slate-500/10 text-slate-400 border-slate-500/30';
}

export default SentimentFeed;
